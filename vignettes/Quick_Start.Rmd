...
base-R, **qeML** or other packages, with the wrappers adding discrimination
...
Care has been taken to design a uniform interface to **dsld** function.
In most cases, the first two arguments are **data**, a data frame, data
table or tibble, and **yName**, the name of the column containing the
response variable Y. Typically there is also an argument **sName**,
specifying the name of the column corresponding to the sensitive
variable S.  Almost all functions are paired with generic **predict**
...
dummies such as **race1black**. Also, while there may be methodological
issues---possible collider effect, "predicting" the past etc.--here we
are simply demonstrating the software.  LSAT scores in this dataset
ranged from 11 to 48 points. 
...
Family income here is reported in quintiles, so see that the difference
...
extra capabilities that **dsldLindar** adds to the base-R **lm**.
Running
...
scores is about 5.9954 points with a standard error of 0.1410. The other
