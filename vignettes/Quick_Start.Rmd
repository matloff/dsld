...
* The classical issue: Are two individuals, identical in all respects X
  ...
  Y? For instance, do men and women of the same
...
also allows the user to specify that interaction terms be added between
X and S.  If say S is race, this enables comparison between Black and
white, Black and Hispanic and so on, at specified X values.
...
**dsldFgrrm** function. It consists of a modified logistic model, with a
LASSO-style shrinking mechanism to limit the effect of S. The degree of
limitation is specified by the **unfairness** argument, a value in (0,1];
smaller values mean more fairness.
...
minorities. The dataset allows us to explore alternatives.
...
Say our dataset has k columns. Each column will correspond to one
vertical section in our graph.  For each row of data, we draw polygonal
line horizontally, with the height at vertical section i between the
value of variable i for that row. (Typically we center and scale each
variable.)

The function **dsldFreqParCoord** displays the m most commonly-appearing
rows from each level of S. Here we try it on the **lsa** data, with m =
75:
