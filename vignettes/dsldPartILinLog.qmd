...
At first glance, it appears that women are faring better than, or at
least equal to, men.  If anything, it is the men who are discrimination
victims.  But upon closer inspection, confidence intervals are rather
iffy.  For case 555, say, the confidence interval for the difference is
$-0.0707 \pm 1.96 \times 0.0679$, or (-0.2038,0.0624).  The wide
interval reflects the fact that our dataset is rather small, n = 1000.

And again, the size of the gender gap seems to depend a lot on the other
factors.  Let's take a look at those comparison cases:
...
Recall our previous example of the German credit data, which consisted
of a total of 21 variables:
...
This last point is especially important.  We do not merely have 19
...
[*Causal discovery* methods may be considered part of this "genre" too.
See Appendix B.]{.column-margin}
selection* problem have been proposed over the years.  The **qeML**
package includes five of them (see the documentation), and there are
myriad others.

Here we will use a method called *Feature Ordering by Conditional
Independence* (FOCI), implement in a CRAN package of the same name, and
wrapped in **qeML** by **qeFOCI** and related functions.  Here are the
key aspects of the method:

* It is *stepwise*:  It add features one at time. If a feature does not
  improve prediction beyond the set already selected, this feature is
  not added.

* The criterion for selecting the feature X~j~ is that Y and X~j~ are
  not independent, given the earlier-selected features.

* There are no *tuning parameters*, values similar to, for instance, k
  in k-NN, that the user must set.

* There are no distributional assumptions.

The stepwise aspect is key By contrast, a random forests algorithm may
measure importance of a variable according to the number of trees in
which it is used for a node split.  This is of interest, but it may
result in too much duplication.

Let's give it a try:

```{r}
gcNoS <- german.credit[,-21]
gcNoY <- german.credit[,-20]
gcNoY$Gender <- as.numeric(gcNoY$Gender == 'Male')
gcNoS$Credit_risk <- as.numeric(gcNoS$Credit_risk == 'GOOD')
qeFOCI(gcNoY,'Gender')$sel
qeFOCI(gcNoS,'Credit_risk')$sel
