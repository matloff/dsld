...
and gender, the model implied a uniform gender, independent of age.  The
...
The curves are no longer parallel; they even cross.
...
Let's predict credit risk.  To keep the example simple, let's just use a few predictors, with S = gender.

```{r}
gct <- gct[,c('Credit_history','Credit_amount','Present_employment_since',
   'Credit_risk','Foreign_worker')]
w <- dsldLogit(gct,'Credit_risk','Foreign_worker',gct[2,1:3],yesYVal='GOOD')
summary(w)
```
