...
interaction here.  The no-interactions model should be adequate.
...
features, etc.]{.column-margin} The exact form of the logistic model
stems from the following setting.  Say we predict gender from age and
wage income.  If the latter two variables have a *bivariate normal*
...
need to specify our desired levels.  And, again, separate logit models
will be run for each level of S.
...
weight of each person.  Faced with a new case of a person 70.2 inches
tall but with unknown weight, we find the people in our training set
whose heights are close to 70.2 inches, and average their weights.  That
average is our predicted weight for the new person, as well as our
estimate for the value of the regression function at height = 70.2.
...
```{R}
w <- dsldML('svcensus',quote(wageinc),'gender',qeMLftnName='qeKNN',opts=list(k=15))
print(w)  
```
...
coefficients.  In particular, look at the one for family income, 0.3009.
The p-value is essentially 0, which classically would be termed "very
highly significant."  But actually, the impact of family income is not
very large.  Here's why:
...
the next 20%, the mean difference in LSAT score rises by only about 1/3
...
This is valid, but may seem odd at first.]{.column-margin} there are
very concerning racial differences here.  Two very similar
...
almost 6 points if one person is black and the other is white.  And thus
the significance test was highly misleading.
...
w <- dsldLogit(lsa,'bar','race1',comparisonPts,'TRUE')
summary(w)   
```
