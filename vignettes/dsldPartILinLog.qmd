...
lsa <- law.school.admissions[,-c(1,10)]
```

There has been concern that the LSAT is heavily influenced by family
income, thus unfair, especially to Black and Latino students, and might
otherwise have racial issues.  Let's investigate, using **dsld**.
...
Quite a bit here. First, let's consider the b~i~, the estimated
coefficients.  In particular, look at the one for family income, 0.3620.
The p-value is essentially 0, which classicly would be termed "very
highly significant."  But actually, it's not very large.  Family income
in this dataset is measured by quintiles.  So this estimated coefficient
says that, for example, if we compare people who grew up in the bottom
20% of income with those who were raised in the next 20%, the mean
difference in LSAT score rises by only about 1/3 of 1 point--on a test
where scores are typically in the 20s, 30s and 40s.
...
So family income is not an important factor after all.  Nevertheless,
there are concerning racial differences here.  Two very similar people,
attending the same quality law school, with the same undergraduate grades, the s
same law school grade, even having the same bar passage status, will have LSAT
scores differing on average by almost 6 points if one person is Black and 
the other is white.

Again, one must be very cautious in drawing conclusions as to causes, as
*hidden confounders* may be at work here.  For instance, though we have
data on undergraduate GPA, we don't know the quality of the undergraduate
institution.  And as noted, it's not clear how exactly this data was collected.
But the results here raise serious concerns.
