...
<!-- gonna remove this for now, due to the lack of iterative methods we use -->
<!-- ::: {.callout-note}
...
from the introduction to this book that to minimize fairness, we must eliminate 
...
deweight values transition from 1 to 0, our utility also **increases** (since the 
...
eliminated. This may result in unexpected trends in later runs of the function 
...
```{r}
############ Demographic Parity #############
dem1 <- dsld::dsldFairTest(data, yName, sName, dsld::dsldQeFairRF, fairness::prop_parity, 
             nReps = 10, deweightPars = list(decile_score=1, priors_count=1))
dem0 <- dsld::dsldFairTest(data, yName, sName, dsld::dsldQeFairRF, fairness::prop_parity, 
...
deweight values = 1 to deweight values = 0, we see that the utility is **increasing** 
...
For our demographic parity results, the distance from 1 larger at the deweight 
value of 0, so our fairness is **decreasing** according to demographic parity.
...
```{r}
############## Equalized Odds ###############
eqod1 <- dsld::dsldFairTest(data, yName, sName, dsld::dsldQeFairRF, fairness::equal_odds, 
             nReps = 10, deweightPars = list(decile_score=1, priors_count=1))
eqod0 <- dsld::dsldFairTest(data, yName, sName, dsld::dsldQeFairRF, fairness::equal_odds, 
             nReps = 10, deweightPars = list(decile_score=0, priors_count=0))
# Deweight Value = 1
print(eqod1)
# Deweight Value = 0
...
**decreased** at a deweight value of 0. However, our utility also seems to be 
**decreasing** based on the increasing misclassification error, which is unexpected 
