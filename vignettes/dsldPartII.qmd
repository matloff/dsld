...
Suppose that decile_score and v_decile_score are the proxies to our sensitive variable 
(race).[Based on results from **dsldDensityByS**]{.column-margin}
...
adjusting the deweight values for decile_score and v_decile_score, we should be able 
to adjust fairness and utility.
...
```{r}
data(compas)
regtools::replicMeans(5,"{rf1 <- dsldQeFairRF(compas, 'two_year_recid', deweightPars = list(decile_score=0.1, v_decile_score=0.1), sName = 'race'); c(rf1$corrsens, rf1$testAcc)}")
regtools::replicMeans(5,"{rf2 <- dsldQeFairRF(compas, 'two_year_recid', deweightPars = list(decile_score=1, v_decile_score=1), sName = 'race'); c(rf2$corrsens, rf2$testAcc)}")
```
...
fairTest <- function(deVal = 1) {
    ...
    # vector of only African-Americans
    AA <- out[out['race'] == 'African-American',]
    # vector of only Caucasians
    ...
    c(mean(AA[,1]), mean(C[,1]), model$testAcc)
}

# output the mean of 500 trials of no deweighting and deweighting decile scores at 10%
replicMeans(100,"{c(fairTest(1),fairTest(.1))}")
```