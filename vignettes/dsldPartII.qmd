...
Given a linear model, it is important to understand how much a feature relates
to the outcome feature. Even more so, when the examined feature is a protected
class like race, gender, and age. A way to measure this association is through
S-correlation. It is possible that the feature we examine, S, may be strongly, 
slightly, or weakly associated with the outcome feature, Y. Understanding how S 
and Y are correlated is meaningful because then we can see the effects S has on 
the prediction model. S-correlation can also be a reason to delve into 
determining why the S feature has such an effect on Y feature.
...
points on measuring utility, maybe a brief background...

#### Test Accuracies (Misclassification Error)
<!-- accuracy results on test sets -->
For classification settings, 
[Reminder: classification settings are prediction applications where Y is 
categorical]{.column-margin}
one simple method to measure the utility of a model 
...
Thus, we can redefine our accuracy and error rate formulas as follows:
$$
...
misclassified, thus affecting the Test Accuracy.
