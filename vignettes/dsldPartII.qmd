...
fairness. The definitions are as follows:
 - **True** means that the actual result and the preedicted result are the same
 - **False** means that the actual result is the opposite of the predicted 
   result
...
So, True-Positive would be a correctly predicted positive outcome, while False-
Negative would be an incorrectly predicted negative outcome (predicted `0`, 
actual was `1`). The rate appended to the end means a ratio with regards to all 
positives or negatives.

Aliases for the above terms are as follows:
...
demoTest <- function(deval, threshold = .5) {
    ...
    # as recidivated
    out <- sapply(split, function(x) mean(x[[1]]))
