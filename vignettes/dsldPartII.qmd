...
```{r}
# FUNC can be any of the fairness measures that the fairness package provides
fairTest <- function(deval, FUNC, cutoff = .5) {
  sink()
  # deweight decile_score and v_decile_score
  deweightPars = list(
    decile_score=deval, 
  ...
  test$probs <- predict(model, test)$probs[,2]
  
  dem <- FUNC(test, yName, sName, 'probs', 
                              outcome_base = "Yes", base="Caucasian",
                              cutoff = cutoff)
  sink()
...
# Demographic Parity
suppressWarnings({
  print(apply(replicate(10, fairTest(1, fairness::dem_parity)), c(1,2), mean))
  print(apply(replicate(10, fairTest(0, fairness::dem_parity)), c(1,2), mean))
})
```
...
# Equalized Odds
suppressWarnings({
  print(apply(replicate(10, fairTest(1, fairness::equal_odds)), c(1,2), mean))
  print(apply(replicate(10, fairTest(0, fairness::equal_odds)), c(1,2), mean))
})
```
