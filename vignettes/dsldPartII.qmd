...
### Prediction
Machine learning is all about prediction. The for the compas data set for 
example, we may want to predict whether or not a person would commit a crime 
again.  We can do that by training a predictive model on the compas data set. 
We're going to use qeML's qeRFranger function for a quick random forest model, 
but there are many, many others, as we will explore later.

```{R}
data <- fairml::compas
yName <- "two_year_recid"
model <- qeML::qeRFranger(data, yName, yesYVal="Yes")
```

Once we have a model, we can give it a "person" and it will tell us it's prediction
on whether or not they will recidivate. 

```{R}
person <- data.frame(
  age=24, juv_fel_count=0, decile_score=1, juv_misd_count=0, juv_other_count=0,
  v_decile_score=1, priors_count=2, sex="Male", race="Caucasian", c_jail_in=0.800, 
  c_jail_out=0.810,  c_offense_date=0.710, screening_date=0.720, 
  in_custody=0.750, out_custody=0.820
)

predict(model, person)
```
Based on the characteristics of this person, the model gives out a "0", which means
the model doesn't think that this person will recidivate.

But we want to measure how fair the model is with regards to one's race.
To start, we can simply count how many cases of "Yes" the model predicts for 
recidivism in respect to each race, given a portion of the data in the compas set.

```{R}
# this code splits the data randomly, leaving 70% for training and 30% for testing
partition <- sample(1:nrow(data))[1:round(nrow(data) * .3)]
train <- data[-partition, ]
test <- data[ partition, ]

model <- qeML::qeRFranger(train, yName, yesYVal="Yes")

prediction <- predict(model, test)
raceRecid <- cbind(as.numeric(prediction$predClasses), test['race'])
split <- split(raceRecid, raceRecid$race)
sapply(split, function(x) sum(x[[1]]))
```
We see that the model has the most "Yes" predictions in the African-American group.
However, this is a crude measure of fairness, as we will see later.
...
[[Paper recently published by Li et al.](https://arxiv.org/pdf/2308.10149.pdf)]{.column-margin}
...
### Short Conclusion
With the descriptions of the approaches deployed for ensuring fair machine 
learning, we hope some intuition behind these algorithms has been imparted 
onto you. Demystifying what happens behind the scenes can enable more people 
to accurately trust and distrust results as necessary. As you deploy your own 
algorithms, ensure that you use this knowledge to best pick the approach that 
not only performs the best, but that you can understand.
