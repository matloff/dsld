...
demoTest <- function(deval, threshold = .5) {
    ...
    split <- split(out, out$race)
    
    # get the mean (the proportion) for each race of those who are classified
    # as recidivated
    props <- mapply(function(x) mean(x[[1]]), split)
    props["Variance"] <- var(props)
    props["MisclassRate"] <- model$testAcc
    props
}
...
African-American            Asian        Caucasian         Hispanic  Native American            Other 
       0.5420749        0.2500000        0.3226087        0.3608333        0.5000000        0.2653333 
        Variance          MisclassRate 
       0.0148852        0.2293333 
attr(,"stderr")
African-American            Asian        Caucasian         Hispanic  Native American            Other 
    0.0025936599     0.0000000000     0.0019498006     0.0017786456     0.0000000000     0.0057777778 
        Variance     MisclassRate 
    0.0003246924     0.0052357325 
```
With deweighting decile_score and v_decile_score at 0:
```
African-American            Asian        Caucasian         Hispanic  Native American            Other 
