...
may involve correlation.  Here's an example:
...
using, say, the **loan_val** and **condo** variables as proxies.
...
and both utility and fairness in the fair ML realm,
...
[Here and below, to keep things simple, we will not use a holdout set.]{.column-margin}
For instance, let's consider our mortgage example, say with k-Nearest
Neighbors as our ML prediction tool:

```{r}
z <- qeKNN(mortgageSE,'deny',holdout=NULL,yesYVal='1') 
# look at the fitted model's probability of denial of the mortgage
# application, i.e. regression estimates
probs <- z$regests
# the variable 'black' is an R factor, need to be numeric
black <- ifelse(mortgageSE$black=='1',1,0)
...
non-classification problems, say predicting wage income:

```{r}
z <- qeKNN(svcensus,'wageinc',holdout=NULL)
gender <- ifelse(svcensus$gender=='male',1,0) 
cor(svcensus$wageinc,gender,method='kendall') 
```

So, there would seem to be only a moderate degree of unfairness, if any.
...
Below is an example using the mortgage data:

```{r}
z <- qeKNN(mortgageSE,'deny',holdout=NULL,yesYVal='1')
# determine which rows were for Black applicants, which not
BlackRows <- which(mortgageSE$black == '1')
NonblackRows <- which(mortgageSE$black == '0')
# regest, the output of qeKNN, is the vector of fitted probabilities
# (recall that for the Y = 0,1 case, the mean reduces to the probability
# of a 1)
BlackProbs <- z$regest[BlackRows]
NonblackProbs <- z$regest[NonblackRows]
...
That's quite a difference!  Overall, about 12% of African-American loan
applicants are denied, versus less than 2% for non-Blacks.
...
In the mortgage data, consider a scenario of two applicants, one Black
and the other nonblack but who otherwise similar.  Say both have their
loan applications denied.  For fairness, we would hope that our ML
algorithm originally assigned both of them the same probability of
denial .  

If for instance the nonblack applicant had been assessed as being of
lower risk for repayment, that would be unfair.  After all, they had similar 
characteristics at the time they applied for the loan, and they had the same 
outcome.  Shouldn't the algorithm have treated them equally?

Equalized Odds requires the quantity

TP / (TP+FN)

to be the same for each sensitive 

(remember, "positive" here m
