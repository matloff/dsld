...
given values of the C and S variables in a linear form.  
...
gender pay gap.  So, let's take the set of confounders C to consist of
age, and for simplicity in this introductory example, not include any
other confounders, such as occupation, and let's not include any other
variables X.
...
```{r}
svcensus1 <- svcensus[,c(1,4,6)]  # age, wage, gender
...
only with sample data from some population, real or conceptual.
.]{.column-margin} For now, note that the estimated b~2~ turns out to be
...
For now, note that the estimated b~2~ turns out to be Open any
professional journal in medicine, sociology, economics and so on, and
you'll many application of this methodology.  But how would one check
that most basic assumption, the linearity of the mean Y for given X, C
and S values?

Typically, linearity is checked graphically.  [Alas, often researchers
proceed without any checking of linearity or other assumptions at
all.]{.column-margin}  A common approach involves plotting the
*residuals*, which are the differences between the fitted line and the Y
values.  Here, though, we use another graphical approach, via
a **dsld** function that may be more informative. 
...
[There is a conditions argument; we have none here, so we just used a
...
The model have mean Y being a linear of function of age, so we  should
expect to see approximate straight lines here.  Yet the relation
certainly looks nonlinear, possibly reflecting [Adding a squared term
does not make our model nonlinear, as it is still linear in the b~i~; if
we, say, double each of those, the entire expression is doubled, the
definition of linearity.]{.column-margin} variable, once for each level
of age discrimination.  We are already investigating one kind of
discrimination here, gender, so again for simplicity let's keep age as a
confounder.  But we must do something about the substantial nonlinearity
we've discovered, and one possible remedy is to add an age^2^ term be
...
So, with this more accurate model, the gender gap actually increased
further.  But we are just getting started!  As noted, we should think
about adding a second confounder, occupation, and later consider
interactions between and gender and age discrimination here.
