...
$$
\frac
{1}{1 + e^{-\beta_0 - \beta_1 age}}
$$

That follows the form of the *logistic function*,
...
$$
\frac
{1}{1 + e^{-\beta_0 - \beta_1 age - \beta_2 wage income}}
$$

Now, what about that assumption of the normal distributions and so on?
Just as the many regression functions in practice are roughly linear, in
predicting binary Y the S-curve model is roughly valid.  Moreover, the
logistic model has two desirable properties for predicting binary Y:

* Its value is between 0 and 1, appropriate for modeling a probability.

* The expression $1 / [1 + e^{-t}]$ is increasing in t.
