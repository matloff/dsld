...
[Python wrappers are included for most functions.]{.column-margin}
...
Consider the census example above.  For simplicity, say we decide to
consider only age as a confounder, so that Y is wage, C consists only of
age, and S is gender.  Older workers tend to have more experience and
thus higher wages, and if there is an age differential in our data, say
with female workers tending to be older, this may mask a gender pay gap.
...
men and M = 0 for women.  The parameters b~i~ are estimated from the
data.

The parameter b~2~, which turns out (see below) to be about $13,000,
is the wage gap, if any.  Here's why:
...
```{r}
dsldConditDisparity(pef,'wageinc','gender','age','age >
0',yLim=c(0,150000)) 
```
