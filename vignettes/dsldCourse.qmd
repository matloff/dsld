...
The with-interactions model is technically

> mean W = $\beta$~0~ + $\beta$~1~ A + $\beta$~2~ M + $\beta$~3~ AM 

So for example, the gender pay gap for 

However, this is a bit abstract, and it is easier (and approximately
equivalent) to simply fit two linear models, one for men and one for
women.
...
not shown, is larger, close to $300.
